...
import type { IpRestrictionDto, UpdateIpRestrictionRequest } from '../types'
...
interface EditIpRestrictionModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  restriction: IpRestrictionDto | null
  onSubmit: (id: string, data: UpdateIpRestrictionRequest) => void
  isLoading?: boolean
}

export function EditIpRestrictionModal({
  open,
  onOpenChange,
  restriction,
  onSubmit,
  isLoading,
}: EditIpRestrictionModalProps) {
  ...
  // Update form when restriction changes
  useEffect(() => {
    if (restriction && open) {
      form.reset({
        type: restriction.type,
        reason: restriction.reason,
        isPermanent: !restriction.expiresAt,
        expiresAt: restriction.expiresAt ? new Date(restriction.expiresAt) : null,
      })
    }
  }, [restriction, open, form])
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>{t('ipRestrictions:edit.title')}</DialogTitle>
            <DialogDescription>
              {t('ipRestrictions:edit.description')}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="ip-address"
                value={restriction.ipAddress}
                disabled
                className="font-mono bg-muted"
              />
              <p className="text-xs text-muted-foreground">
                {t('ipRestrictions:edit.ipAddressReadonly')}
              </p>
            </div>
